...
data = pd.read_csv('US_WildFires_Dec20_updated.csv')
import http.client

conn = http.client.HTTPSConnection("trailapi-trailapi.p.rapidapi.com")

headers = {
    'x-rapidapi-host': "trailapi-trailapi.p.rapidapi.com",
    'x-rapidapi-key': "Aysdfn12823b123nasd9CAS"
    }

conn.request("GET", "/trails/explore/?lat=null&lon=null", headers=headers)
...
wildfire = data[['Start_Lat','Start_Lng','Severity']]
...
def getTrafficInfoSingle(xCoord, yCoord):
    ...
    return JF/n
def compute_total_wildfires(x1, y1):
    totalWildfires = 0
    for wildfire in wildfire.itertuples():
        if (abs(x1 - wildfire.Start_Lat) < 0.01 or abs(y1 - wildfire.Start_Lng) < 0.01):
            if get_distance(x1, wildfire.Start_Lat, y1, wildfire.Start_Lng) <= 5:
                totalCrashes+=1
    return totalWildfires
...
def gettotalWildfires(paths, n = 5):
    computed = 0
    totalWildfires = []
    for path in paths[:n]:
        crashes = 0
        for coord in path:
            crashes+=compute_total_wildfires(coord['lat'], coord['lng'])
        computed+=1
        totalWildfires.append(wildfire)
    return totalWildfires
def getHikingdiff(paths, n = 5):
    jf_ = []
    for path in paths[:n]:
        totalJF = 0
        n = 0
